...
async function run() {
  ...
  console.log("Fetching season episodes from:", url);
  const res = await fetch(url, {
    headers: { Authorization: `Bearer ${token}`, "User-Agent": USER_AGENT }
  });
  if (res.ok) {
    const data = await res.json();
    console.log(`Success! Fetched ${data.data?.length} episodes in season.`);
    if (data.data?.length > 0) {
      console.log("First episode title:", data.data[0].title);
    }
  } else {
