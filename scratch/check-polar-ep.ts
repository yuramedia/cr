...
async function run() {
    ...
    const episodeId = "GF00366879JA1P"
    const url = getProxyUrl(`${API_BASE}/content/v2/cms/objects/${episodeId}`)
    ...
    if (res.ok) {
        ...
        if (ep) {
            console.log("EPISODE DETAILS:", JSON.stringify(ep, null, 2))
        } else {
            console.log("NO EPISODE DATA", data)
        }
    } else {
