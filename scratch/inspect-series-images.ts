...
async function run() {
  ...
  const seriesId = "GT00371860"; // Mistress Kanan
  const url = getProxyUrl(`${API_BASE}/content/v2/cms/series/${seriesId}`);
  ...
  if (res.ok) {
    ...
    if (series) {
      ...
      console.log("SERIES IMAGES DETAILS:", JSON.stringify(series.images, null, 2));
    }
  }
}
